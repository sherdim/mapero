...
class Module(HasTraits):
	...
	def __init__(self, **traits):
		...
		self._process_thread = ProcessThread(self)
		print "creado el hilo"

		#self.logs = {'error': '', 'warning': '', 'info': '', 'debug': ''}
		#self.log('info', "Module %s created", self.__class__.__name__)

	def __del__(self):
		print "Module %s deleted" % self.__class__.__name__

	def log(self, level, msg, args = ''):
		if level == 'error' or level == 'warning' or level == 'info' or level == 'debug':
			if args:
				self.logs[level]=self.logs[level]+ msg % args + '\n'
			else:
				self.logs[level]=self.logs[level] + msg + '\n'
	...
	def stop_module(self):
		self.stop()
		print 'Module %s stoped' % self.__class__.__name__

...
from enthought.traits.api import Any, Array, Enum, false, Float, Instance, Property, true, List
...
#            self.value_mappers.on_trait_change(self._update_mappers, "updated")
	...
	print "_gather_points terminado"
	    ...
	    print "not use_downsampling"
            pts = BaseMultiLinePlot._downsample(self)
        self._render(gc, pts)
        return
	...
	h_sign = self.height/lmappers

	y_top = y

	for y_mapper in self.value_mappers:
	    y_mapper.low_pos = y_top
	    y_mapper.high_pos = y_top + h_sign
	    y_top += h_sign
